 ...
 */
class BlogCategory extends RedActiveRecord
{
    /**
    ...
    public function renderDropDownList($default = "")
    {
        $res = $this->findAll();
        if (!empty($default)) {
            $ret = array($default);
    ...
    {
        $cache = Yii::app()->cache;
        $key = 'app.models.BlogCategory.renderArchive';
        if (($archive = $cache->get($key)) == false) {
            $archive = $this->findAll('post_count>0');
